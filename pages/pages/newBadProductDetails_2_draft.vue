			...
			getOldReasonData(AbnormalNoDetailsID) {
				this.$request.post({
					...
					if (res.data.result) {
						...
						_this.$nextTick(() => {
							_this.$set(_this.tableData, 1, newChildrens);
						})
					...
					uni.showModal({
						...
						success: function(res) {
							if (res.confirm) {
								...
								let childrens = [];
								let flag = 0;
								_this.tableData[0].some(x => {
									if (!x.BadQty || parseFloat(x.BadQty) == 0) {
										flag = 1;
										return true;
									}
									let childrens2 = [];
								...
								if (flag == 1) {
									...
									// 获取已经被删除了的旧产品数据
									let newList = _this.temporaryProductData.filter(c => !_this.tableData[0]
										.some(
									...
									let newObj = JSON.parse(JSON.stringify(_this.headForm));
									_this.$set(newObj, 'childrens', childrens);
									_this.$set(newObj, 'AbnormalStatus', val);
